...
async def all_products(request: Request) -> Response:
    async with aiohttp.ClientSession() as session:
        ...
        if products in pending:
            ...
            product_response = product_response[:10]
            ...
            return web.json_response({'cart_items': cart_item_count, 'favorite_items': favorite_item_count, 'products': product_results})    
...
async def get_products_with_inventory(session: ClientSession, product_response) -> List[Dict]:
    ...
    inventory_tasks_to_product_id = {asyncio.create_task(inventory_circuit.request(session, product['product_id'])): product['product_id'] for product in product_response}
    # inventory_tasks_to_product_id = {asyncio.create_task(get_inventory(session, product['product_id'])): product['product_id'] for product in product_response}
